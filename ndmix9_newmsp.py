...
def outfile_stat(Symm_stat,sim_num,model_num):	
	outfile = open('data/stat/symmetry_stat_%s_%s_%s' %(model_num,sim_num,os.environ["SLURM_NODEID"]), 'a')
...
def outmatrix(Symm_stat, sim_num, model_num):
	...
	with open('data/mat/symmetry_matrix_%s_%s_%s' %(model_num,sim_num,os.environ["SLURM_NODEID"]),'wb') as f:
...
# matrix = outmatrix()
def simulateFromDist(model_num, sim_num):
	...
	#print(rg.uniform_int(2000)[0]*0.0001)
