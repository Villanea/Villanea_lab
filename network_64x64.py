...
for i, file in enumerate(glob.glob('data/mat_preseed/symmetry_matrix_1*')):
...
for i, file in enumerate(glob.glob('data/mat_preseed/symmetry_matrix_2*')):
...
for i, file in enumerate(glob.glob('data/mat_preseed/symmetry_matrix_3*')):
	#write a line of zero to the file
	EU_AS = np.loadtxt(file)
	# #add columns to make it 101x101
	# temp_arr = np.zeros((EU_AS.shape[0], 101))
...
for i, file in enumerate(glob.glob('data/mat_preseed/symmetry_matrix_4*')):
	#write a line of zero to the file
	EU_AS = np.loadtxt(file)
	# #add columns to make it 101x101
	# temp_arr = np.zeros((EU_AS.shape[0], 101))
...
for i, file in enumerate(glob.glob('data/mat_preseed/symmetry_matrix_5*')):
	#write a line of zero to the file
	EU_AS = np.loadtxt(file)
	# #add columns to make it 101x101
	# temp_arr = np.zeros((EU_AS.shape[0], 101))
...
dense = model.fit(x=data_flat, y=labels_random, epochs=150, verbose=1, batch_size=64, validation_split=0.1)
