...
outfile = open('outfile_sim%s.bed' %(1), 'w+')
outfile.close()
def neanderthal_admixture_model(num_eu=170,num_as=394,num_nean = 1,anc_time=900,mix_time1=2000,mix_time2=1000,mix_time3=1000,mix_time4=1000,split_time_1=120000,split_time_2=2300,split_time_3=1500,f1=0.022,f2=0.00,f3=0.00,f4=0.20,Ne0=10000,Ne1=2500,Ne2=10000,mu=1.5e-8,window_size = 100000,num_SNP = 1,num_rep=1,coverage=False):
    for chr in range(1,23):
        ...
        for sim in sims: 
            ...
            cur_sim += 1
            for tree in sim.trees(): # JI- call this the "tree loop"
                F_int = tree.get_interval()
                while cur_site >= F_int[0] and cur_site < F_int[1]: # JI - call this the "site loop"
                    cur_node = len(samples)-1  # JI - move this line through line 90 to inside of tree loop before site loop (to between line 79 and 80)?
                    ...
                    for leaf in tree.leaves(cur_node):  
                        if tree.get_population(leaf)== 0:
                            ...
                            N_freq_AS += 1 # JI - last line to move out of site loop
                    pos.append(cur_site)
                    ...
                    cur_end += window_size
                    # print cur_end # JI - print is generally super slow, avoid if possible, esp. in often repeated loops
                    # # print last # JI - remove print if possible
                    if cur_end > last:
                        break
                    cur_win += 1
                    # print cur_win # JI - remove print if possible
                    cur_site = int(((cur_start+cur_end)+1)/2.0) #random.randint(cur_start,cur_end)
                    # print cur_site # JI - remove print if possible
        outfile = open('outfile_sim%s.bed' %(1), 'a')
        for line in range(0,len(freq_AS)): # JI - less important than the other issues, but it may be better to write lines out to the file within the site loop, instead of waiting until the end.  That way you don't need a list of all lines, but rather only need to keep one line in memory at once. This will reduce memory usage overhead.
            outfile.write(chrom)
            outfile.write('\t')
...
#bedops
os.system("sort-bed outfile_sim%s.bed > outfile_sim%s_sorted.bed" %(1))
os.system("rm outfile_sim%s.bed" %(1))
os.system("bedops --element-of 1 outfile_sim%s_sorted.bed human_genome_mask_sorted.bed > outfile_sim%s_masked.bed" %(1))
os.system("rm outfile_sim%s_sorted.bed" %(1))
...
#sys_stat
def symmetry_stat():
    EU = np.genfromtxt('outfile_sim%s_masked.bed' %(1), usecols=3)
    AS = np.genfromtxt('outfile_sim%s_masked.bed' %(1), usecols=4)
    
    # delete sim file
    # os.system("rm outfile_sim%s_masked.bed" %(1))
